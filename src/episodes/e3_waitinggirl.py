...
def sc_satellite_line(w: World):
    ...
    return w.scene("衛星回線",
            ...
            yuri.do("それらに対し、いちいち「押収物なし」と$kudoに記録させる",
            ...
            yuri.do("天井を見上げ、ゴーグルに小説を呼び出そうとして、$Sはそれを止めた", "彼女のことが気になったからだ"),
            yuri.do("彼女は幽霊、あるいは霊的現象と呼ばれる存在だった", "図書館を出てすぐに$kudoに解析してもらったからそれは間違いない",
            ...
            yuri.do("このような現象に今まで何度か遭遇したことはあったが、それでも$jasのようにはっきりとした輪郭と反応を持っているものは珍しかった"),
            w.br(),
            kudo.talk("$yuri、おはよう"),
            yuri.do("久しぶりに寝過ごしたらしい"),
            yuri.do("目を開けると$kudoがベッドの上で膝を抱えている彼を覗き込んでいた"),
            yuri.talk("ああ、失態だ", "ところで通信は終わった？"),
...
def sc_mosque_and_flower(w: World):
    ...
    return w.scene("モスクと花",
            w.cmd.change_stage("mosque"),
            w.cmd.change_time("midmorning"),
            w.symbol("　　　　５"),
            yuri.do("図書館を訪れるのは後回しにして$Sは$kudoと共に他の生き残っていそうな施設を見て回ることにした"),
            yuri.do("何もかもが息絶えた、と云われている『$w_walpurgis』を超えた現在、それでもまだ僅かな人類は生き延びていた",
            ...
            yuri.do("学校から五キロくらい離れた場所に真っ白な建物があった", "家ではない"),
            ...
            yuri.do("だが犬型ドローンはそれには何も答えず、記録の為に施設内を歩き回る"),
            yuri.do("一分も見れば全てに目を通せるような質素な空間だった", "予想通りだ"),
            yuri.talk("何もない"),
            yuri.do("$yuri以外の$w_archiverの大半が、その一言を記入する",
                "この世界では人が作ったものは現在機能しているものを除いてほぼ失われてしまっていた",
                "代わりにかつて「自然」と呼んでいたその一部である植物や昆虫類が探す必要がないほどに増えてしまっている"),
            yuri.talk("収穫はなしっと"),
            yuri.do("入り口に戻ると、先に出た$kudoが盛り上がったアスファルトの上でこちらを振り返っていた", "さっさと次の場所を調査しようというつもりだろう"),
            yuri.talk("ねえ$kudo。この花は何という名前だろう"),
            yuri.do("それは建物の左手側の空間に造られていた", "煉瓦で囲った中から植物が溢れるように茎や葉を伸ばしていた",
                "$Sが注目したのはその中で一輪だけ咲いている、小さな白い花だ",
                "鬱蒼と伸びる緑の中で星のような五枚の花弁を持つそれはあの図書館に唯一残る彼女の姿のようでもあった"),
            kudo.talk("記録した", "後で問い合わせる"),
            kudo.do("戻ってきた$kudoは目を向けて撮影すると、いつまでもそこを立ち去ろうとしない$yuriに改めてその視線を向ける"),
            kudo.talk("どうした？"),
...
def sc_herwork(w: World):
    ...
    return w.scene("彼女の仕事",
            ...
            yuri.do("学校の備品倉庫を発見し、そこにあった自家発電機を動かしてみるという$kudoと分かれ、",
                ...
                "ただ$w_ghostは不安定な存在で、既に消失している可能性も考慮しつつ彼は玄関を潜った"),
            jas.talk("$yuriさん、ですね"),
            yuri.do("入ってすぐ正面で、眼鏡の彼女は立っていた"),
            yuri.talk("ええ、はい"),
            yuri.do("自分の名前を口にしたことに多少驚きはあったが、昨日会話したことで回路に若干の影響があったのだろうと納得させる",
            ...
            yuri.talk("あの、すみません"),
            jas.talk("何でしょうか"),
            jas.do("彼女は振り返ることなく本棚が本来立っていた場所で指差しをしながら何かの確認をしている"),
            yuri.talk("こちらの図書館について、少し教えてもらえますか？"),
            jas.talk("図書館について、ですか？"),
            yuri.talk("はい"),
            jas.do("彼女は作業を中止して$yuriの方を向き、顎に人差し指を当てて少し考え込む素振りを見せると、"),
...
def sc_lookupmoon(w: World):
    ...
    return w.scene("月を見上げて",
            ...
            kudo.do("$kudoはバッテリィがもったいないからとまだ省エネモードのまま、屋上で今日も衛星通信を試みている",
                "大半の衛星が機能停止してただのデブリになりはて、僅かに送受信可能なものは分厚い雲の隙間を通すような微弱な電波の交流しか行えないらしい"),
            yuri.talk("雨、か"),
            ...
            kudo.talk("$meは植物ではない", "雨は残念だ", "その意見には同意する"),
            ...
            yuri.do("そんな時、彼以外の$w_archiverがどういった過ごし方をしているのか知らないが、$Sは必要な資料を読み込むか、",
                "$kudoに記録してもらった映像を見直して調査レポートの下書きをするか、", "好きな小説を読むかぐらいしか選択肢を持っていなかった"),
            yuri.do("最初はゴーグルに今後の環境浄化予測の資料を呼び出して眺めていたが、一時間もすれば飽きて、結局いつも通り小説を開いた",
                "寝転びながら視線の動きでページを捲る",
            ...
            yuri.do("手持ちのライブラリにはカフカやカミュ、ヘッセといったヨーロッパの作家から、トルストイ、ドストエフスキーといったロシアの文豪の名も並ぶ",
...
def sc_hermind(w: World):
    ...
    return w.scene("彼女の気持ち",
            ...
            yuri.talk("待っているんですね、彼のこと", "$exfull_andreさんはどんな方なんですか？"),
            jas.talk("どんな……"),
            ...
            yuri.do("見渡した書架はどれも倒れ、朽ち、そこに僅かに残る本だったものたちも中身はない",
                "本はどの現場でも腐敗、腐食、破壊されてしまっていた"),
            jas.talk("本は、宝物だと思います", "本に書かれているのは物語だけではありません", "そこには様々な人の思いが溢れているんです"),
            jas.do("彼女はそう言って何かを思い出すように虚空を見る"),
            yuri.talk("分かりました", "もう一日待ちます"),
            jas.talk("すみません"),
            yuri.do("建物を出た彼は空を見上げた", "分厚いガス雲の、更に向こう側だ"),
            yuri.do("そこにはもう、月面基地は存在しない", "十年も前に砕け、その大半はこの大地へと降り注いだのだ", "当時の映像記録はライブラリで一度見たが、正にこの世の終わりと呼ぶに相応しい映像だった"),
...
def ep_waiting_girl(w: World):
    return w.episode("待ち続ける女性",
            ...
            sc_lookupmoon(w),
            sc_hermind(w),
            outline="3.いつまで待っても本を借りに男はやってこない。ユーリィは街を調査し、ある情報を手に入れた",
            )
