...
def sc_perished_world(w: World):
    yuri, kudo = w.get("yuri"), w.get("kudo")
    return w.scene("滅びた世界で",
            w.cmd.change_camera("yuri"),
            w.cmd.change_stage("Jakarta"),
            ...
            yuri.do("しかし見事な着水でブーツの上まで濁ったものが飛び上がり、彼はマフラーに失意の吐息をぶつけた"),
            ...
            yuri.do("かつて犬と呼ばれていた動物に似せて作られた、金属製の自律機械だ",
            ...
            yuri.talk("確かに成功する確率を高く見積もりすぎたとは思うが、それについて議論する為に貴重な$energyを消費するつもりはないよ",
                ...
                "いくらこの$w_archiverが正確なデータを求められる仕事とはいえ、常にそれが必要とされている訳じゃないんだよ？"),
            yuri.do("$Sは言い聞かせるように彼を見たが、しゅっと伸びた鼻筋もピンと立ったままの耳も瞼のない目も、何の変化も示さない",
                "いつだって彼は無表情だ", "当初はそういった機能は記録用ドローンには必要ないと判断されていたのだろう"),
            yuri.talk("それよりそろそろ今日の寝床を探したい"),
            yuri.do("見上げた空はグレィの筋状の雲が幾層にも重なり、日差しはない", "&"),
            yuri.think("$Sにとって空といえばこんな風に分厚い雲で塞がれているか、夜闇によって漆黒で塗りつぶされているかのどちらかでしかなかった", "&"),
            yuri.do("地上に出たからには一度くらいは青空というものを拝んでみたいと思っているがまだその機会は訪れない",
                "おそらく今後もないと予測されるが、楽観的観測をするならば「いつかは見られる」と$Sは考えている"),
            yuri.talk("$kudo"),
            yuri.do("記録用犬型ドローンの名を呼び、$Sは改めて、もう使えなくなったアスファルトの片側二車線を見やった",
                "道路脇にはかつて鉄筋コンクリートのビルが立ち並んでいたと思われるが、その一割程度しか残骸がない",
                "多くが基礎部分のみ残存し、一階、あるいは二階まで残っていれば良い方だ", "&"),
            yuri.look("おそらくはこの街のメインストリートだったと考えられる",
            ...
            yuri.do("半分冗談のつもりで$kudoに言ったが、彼は小さく頷くと三センチほど隙間が開いていたフロント部分に前足を器用に突っ込んで開け、中を覗き込む"),
            kudo.talk("盗難に遭っている"),
            ...
            yuri.do("それでも何も答えない犬型ドローンの背中を見て小さく首を竦めると、$Sは彼の後に続いた"),
            )
...
def sc_old_school(w: World):
    ...
    return w.scene("壊れた学校",
            ...
            kudo.do("$Sは何か言おうとしたが咄嗟に$yuriが先に行くよう促したから黙ったまま、教室の一つに入った"),
            ...
            yuri.do("調べてみたが特に使えそうなものはなく、$kudoと共に廊下へと出る"),
            yuri.do("同じようにどの教室も役立ちそうなものは残っていなかったが、職員用の小さな部屋には窓がなかったお陰だろうか、",
                "パイプ製の簡易ベッドが固くなったマットレスと共に見つかった"),
            yuri.talk("久々に屋内で休めそうだよ、$kudo"),
            yuri.do("上に乗って強度を確かめながらそう言うと、クロゼットの中身を確認していた$kudoは一度顔を$Sに向け、"),
            ...
            kudo.do("ただそれらがどんなに高性能であったとしても向こう百年は発電することは叶わない",
                "そう推定されている", "$yuriは当時の状況をデータでしか知らないが、完全防護服と暗視ゴーグルなしに外を出歩けるようになったのがこの五年程と言われていた"),
            kudo.talk("自家発電機構が存在する可能性を希望する"),
            kudo.do("そう言って屋上の縁まで行くと彼は首をぐるりと動かして周辺状況を観測した",
            ...
            yuri.do("自分の方を見た犬型ドローンは既にモードを変更したようで、目の発光が青色に変化して薄くなり、何も言わずにドアへと歩いて向かう"),
            yuri.think("仕方ないな",
                    "そう独りごちると、$Sは彼に続く", "&"),
            ...
            yuri.do("人はいない", "住んでいる動物もいない", "&"),
            yuri.do("水道や電気は当然通じていないし、非常食の蓄えも見つけられなかった"),
            yuri.do("だからといって落胆はない",
            ...
            yuri.do("$Sはベッドに座り、バックパックから一日分のカロリーと栄養素を固めた歯が折れそうなスティックを取り出す",
...
def ep_ghostworld(w: World):
    return w.episode("滅びた世界",
            ...
            sc_perished_world(w),
            sc_old_school(w),
            outline="1.滅びた世界で記録士をやっているユーリィは、犬型ドローンと共に街の調査に訪れる",
            )
